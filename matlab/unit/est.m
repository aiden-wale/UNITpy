...
if nargin==0,
 try
  west = which('est');
  i = strfind(west,'est.m');
  addpath([west(1:i-1) '../gui_java']);
  unitGuiJava;
  return;
 catch
  disp('Sorry, no GUI available')
 end
elseif nargin<1,
 error('Need to specify data (Z)!');
elseif nargin<2,
    %Assume that the user has only supplied data so try to guess an
    %appropriate model order via subspace algorithm
 Z    = startZ(Z);
 m.nx = min(20,ceil(Z.Ny/10));
 gsid = subspace(Z,m);
 lsin = length(gsid.sing); 
 vv   = [linspace(0,(lsin-1)/lsin,lsin) ; gsid.sing(:)'/gsid.sing(1)];
 [mv,mi] = min(sum(vv.*vv));
 M.A  = mi+1;
 OPT  = [];
elseif nargin<3,
...
%--------------------------------------------------------------------------
%  Unspecified data components->defaults                  DATA
%--------------------------------------------------------------------------

Z   = startZ(Z);

%--------------------------------------------------------------------------
%  Unspecified model structure components->defaults       MODEL
%--------------------------------------------------------------------------

M  = startM(Z,M);

%--------------------------------------------------------------------------
%  Unspecified estimation options->defaults               ALGORITHM
%--------------------------------------------------------------------------

OPT = startOPT(OPT,M);

%--------------------------------------------------------------------------
%  Determine what estimate initialisation is required, and what
%  estimation algorithm should be used.
%--------------------------------------------------------------------------

ep  = estmap(Z,M,OPT);

%--------------------------------------------------------------------------
%  Display start of estimation process
%--------------------------------------------------------------------------

if OPT.dsp,
 dblines='======================================================================================';
 udisp([sprintf('\n') dblines],gui,guih);
 udisp('START ESTIMATION PROCESS:',gui,guih);
 udisp(['Estimating parameters for "' upper(M.type) '" model structure using "' lower(M.op) '" operator.'],gui,guih);
 udisp(ep.modelEquations,gui,guih);
 udisp('INITIALISATION:',gui,guih);
end

%--------------------------------------------------------------------------
%  Initialise Nonlinear parts of model if necessary
%--------------------------------------------------------------------------

if ~isempty(ep.startNL),
 M = feval(ep.startNL,Z,M); %calling startNL
elseif OPT.dsp,
 udisp('No initialisation of Nonlinear structure required.',gui,guih);
end

%--------------------------------------------------------------------------
%  Initialise estimate of system dynamics if necessary
%--------------------------------------------------------------------------

if ~isempty(ep.startG),
 M = feval(ep.startG,Z,M,OPT); %calling startG
elseif OPT.dsp,
 udisp('No initialisation of model dynamics required.',gui,guih);
end

%--------------------------------------------------------------------------
%  Initialise noise model if necessary
%--------------------------------------------------------------------------

if ~isempty(ep.startH),
 M = feval(ep.startH,Z,M,OPT); %calling startH
elseif OPT.dsp,
 udisp('No initialisation of noise dynamics required.',gui,guih);
end
...
if ~isempty(ep.alg),
 M = feval(ep.alg,Z,M,OPT); % Call main alg, e.g. gn or em
elseif OPT.dsp,
 udisp('No main algorithm required.',gui,guih);
end

%--------------------------------------------------------------------------
%  Fill in all components of returned (estimated) model structure
%--------------------------------------------------------------------------

if ~isempty(ep.finishM),
 G = feval(M.finishM,Z,M,OPT); 
elseif OPT.dsp,
 udisp('No model finishing required.',gui,guih);
end


%--------------------------------------------------------------------------
%  Display end of estimation process
%--------------------------------------------------------------------------

if OPT.dsp,
 udisp('END ESTIMATION PROCESS',gui,guih);
 udisp([dblines sprintf('\n')],gui,guih);
end
