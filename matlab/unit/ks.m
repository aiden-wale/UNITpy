...
% Put unset options to defaults
if nargin<3, OPT = []; end
OPT = startOPT(OPT);

% Extract data and with dimensions
[y,u,p,m,N] = Z2data(Z);
y = y';

% If we have no measurements then we can't filter or smooth
if p<1,
...
elseif isfield(M,'ss')
 ...
 n = size(M.ss.A,1);      % Get state dimension
...
% Check to see if there is an exogenous input and adjust everything else to suit
if m>0,
