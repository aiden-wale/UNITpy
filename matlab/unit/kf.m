%
% KF. This function is the square-root implementation of the Kalman
% Filter as per
%
...
% If we have no measurements then we cant filter
if p<1,
...
if m<1 & strfind(M.type,'bilin'), % If there is no input, then the system is not bilinear between input and state, which is all we support
 M.type = 'ss'; 
end
...
% Copy all details of model into output
G = M;  

% Set OPT.smoothing so that only filtering is done
OPT.smoothing = 0;
