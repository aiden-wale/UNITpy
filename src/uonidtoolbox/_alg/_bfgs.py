...
def bfgs(Z, M=unit._struct(), OPT=unit._struct()):
...
def _bfgs_impl(costfcn, theta0, maxiter=100, disp=1):
    ...
    if disp: 
        infotable = unit._optimisation.IterationInfoPrinter(['iter#','cost','Newton decrement','alpha'], [5, 13, 17, 13])
        infotable.printHeader()
        infotable.printDataRow([0, f, '-', '-'])
    #endif
    ...
    for it in range(1, maxiter+1):
        ...
        for i in range(0, 51):
            ...
            if np.isfinite(f_n) and (f_n < f + c1*alpha*pTg): # Wolfe condition 1
                break
        ...
        if disp: infotable.printDataRow([it, f_n, pTg, alpha])
    #endfor
    if disp and it >= maxiter:
        infotable.finishTable()
        unit._utils.udisp(f"Reached max iterations with final cost: {f_n:<10.5e}\n")
    #endif
